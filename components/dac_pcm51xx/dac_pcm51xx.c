 ...
 *
 * Datasheet: https://www.ti.com/lit/ds/symlink/pcm5122.pdf (§9.6, page 0
...
#define PCM51XX_RESET_ALL      0x11 // RSTR registers + RSTM modules, auto-clearing
...
#define PCM51XX_IGNORE_MCLK    0x08 // IDCM: ignore MCLK missing detection
...
static void set_enable_pin(bool on) {
#if CONFIG_DAC_ENABLE_GPIO >= 0
  gpio_set_level(CONFIG_DAC_ENABLE_GPIO, on ? 1 : 0);
#else
  (void)on;
#endif
}
...
static void apply_volume_locked(void) {
  ...
  ESP_LOGI(TAG, "Volume: AirPlay %.1f dB -> code 0x%02X%s", a, code,
           code == 0xFF ? " (mute)" : "");
  wr(PCM51XX_REG_VOL_L, (uint8_t)code);
  wr(PCM51XX_REG_VOL_R, (uint8_t)code);
}
...
static esp_err_t pcm51xx_init(void *i2c_bus) {
  ...
  if (gerr != ESP_OK) {
    ESP_LOGE(TAG, "Failed to configure DAC enable GPIO %d", CONFIG_DAC_ENABLE_GPIO);
    return gerr;
  }
  gpio_set_level(CONFIG_DAC_ENABLE_GPIO, 0);
  ESP_LOGI(TAG, "DAC enable GPIO %d initialized (muted)", CONFIG_DAC_ENABLE_GPIO);
#endif
  ...
  if (err != ESP_OK) {
    ...
    for (size_t i = 0; i < sizeof(candidates); i++) {
      if (i2c_master_probe(s_bus, candidates[i], I2C_PROBE_TIMEOUT_MS) == ESP_OK) {
        addr = candidates[i];
        err = ESP_OK;
        break;
      }
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Could not add device @0x%02X: %s", addr, esp_err_to_name(err));
    return err;
  }
  ESP_LOGI(TAG, "PCM512x detected @0x%02X", addr);

  xSemaphoreTake(s_mutex, portMAX_DELAY);
  // The PCM512x is not reset by an ESP32 reset, so start from a known state:
  // park it in standby and reset registers and DSP modules (clears any
  // coefficient/program RAM left behind by a previous firmware), then
  // configure in standby and bring the DAC up muted. Volume is set on the
  // way so the first unmute already has the right level.
  wr(PCM51XX_REG_PAGE, 0x00);
  wr(PCM51XX_REG_MUTE, PCM51XX_MUTE_BOTH);
  wr(PCM51XX_REG_POWER, PCM51XX_POWER_STANDBY);
  wr(PCM51XX_REG_RESET, PCM51XX_RESET_ALL);
  vTaskDelay(pdMS_TO_TICKS(20));
  wr(PCM51XX_REG_PAGE, 0x00);
  wr(PCM51XX_REG_MUTE, PCM51XX_MUTE_BOTH);
  wr(PCM51XX_REG_POWER, PCM51XX_POWER_STANDBY);
  wr(PCM51XX_REG_PLL_REF, PCM51XX_PLL_REF_BCK);
  wr(PCM51XX_REG_IGNORE_ERR, PCM51XX_IGNORE_MCLK);
  wr(PCM51XX_REG_DAC_ROUTING, PCM51XX_ROUTE_STEREO);
  apply_volume_locked();
  wr(PCM51XX_REG_POWER, PCM51XX_POWER_ACTIVE);
...
static void pcm51xx_set_power_mode(dac_power_mode_t mode) {
  ...
  case DAC_POWER_ON:
    ...
    vTaskDelay(pdMS_TO_TICKS(20));
    wr(PCM51XX_REG_MUTE, PCM51XX_UNMUTE);
    set_enable_pin(true);
    break;
  case DAC_POWER_STANDBY:
    set_enable_pin(false);
    wr(PCM51XX_REG_MUTE, PCM51XX_MUTE_BOTH);
    // Stay active so the next unmute is instant; the chip idles at a few mW.
    break;
  case DAC_POWER_OFF:
  default:
    set_enable_pin(false);
